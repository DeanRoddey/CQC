...
function doInit()
{
    ...
    //  help content under /Web2/CQCDocs
    //
    // helpRoot = window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/"));
    helpRoot = "/Web2/CQCDocs";
    ...
    if (history.replaceState) {
        ...
        var initURL = helpRoot + "/CQCDocs.html?topic="
    ...
    var topicURL = helpRoot + tarTopic + "/CQCTopicIndex.html";
    ...
    window.onpopstate = function(event) {
        ...
        if (newState) {
            ...
            var topicURL = helpRoot + newTopic + "/CQCTopicIndex.html";
            var pageURL = helpRoot + pagePath + ".html";
...
function loadRightSide(topicDir, pageToLoad, bookMark)
{
    ...
    if (history.pushState) {
        ...
        var newURL = helpRoot + "/CQCDocs.html?topic="
                      + topicDir + "&page=" + pageToLoad;

        if (bookMark !== "")
            newURL += "&bookmark=" + bookMark;

        history.pushState(stateObj, "CQC Topic", newURL);
    }
...
function loadDynDiv(topicDir, topicPage)
{
    ...
    var topicURL = helpRoot + topicDir + "/CQCTopicIndex.html";
    var pageURL = helpRoot + pagePath + ".html";

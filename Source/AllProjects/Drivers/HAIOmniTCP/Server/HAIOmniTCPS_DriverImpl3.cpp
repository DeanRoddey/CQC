//
// FILE NAME: HAIOmniTCPS_DriverImpl3.cpp
//
// AUTHOR: Dean Roddey
//
// CREATED: 05/16/2008
...
//
tCIDLib::TBoolean THAIOmniTCPSDriver::bCreateSession()
{
    ...
    try
    {
        ...
        SendPacket(kHAIOmniTCPS::c1PackType_ReqNewSess);
        ...
        if (c1PacketType != kHAIOmniTCPS::c1PackType_AckNewSess)
        {
            if (eVerboseLevel() >= tCQCKit::EVerboseLvls::Medium)
            {
                ...
                (
                    CID_FILE
                    , CID_LINE
                    , kOmniTCPSErrs::errcProto_NoStartSesAck
        ...
        if (c4MsgBytes != 7)
        {
            ...
            (
                CID_FILE
                , CID_LINE
                , kOmniTCPSErrs::errcProto_BadSessAckSize
                , tCIDLib::ESeverities::Failed
                , tCIDLib::EErrClasses::Format
        ...
        SendPacket(mbufIn.pc1Data() + 2, kHAIOmniTCPS::c1PackType_ReqSecSess, 5);
        ...
        if (c1PacketType != kHAIOmniTCPS::c1PackType_AckSecSess)
        {
            if (eVerboseLevel() >= tCQCKit::EVerboseLvls::Medium)
            {
                facHAIOmniTCPS().ThrowErr
                (
                    CID_FILE
                    , CID_LINE
                    , kOmniTCPSErrs::errcProto_NoSecSesAck
                    , tCIDLib::ESeverities::Failed
                    , tCIDLib::EErrClasses::Format
                    , TCardinal(c1PacketType)
                );
...
//
tCIDLib::TBoolean
THAIOmniTCPSDriver::bGetPacket(         TMemBuf&            mbufToFill
                                ,       tCIDLib::TCard1&    c1PacketType
                                ,       tCIDLib::TCard4&    c4MsgBytes
                                ...
                                , const tCIDLib::TBoolean   bOmniMsg)
{
    ...
    if (!m_sockOmni.bWaitForDataReadyMS(c4WaitFor))
    {
        ...
        (
            CID_FILE
            , CID_LINE
            , kOmniTCPSErrs::errcProto_Timeout
            , tCIDLib::ESeverities::Failed
     ...
     else
    {
        ...
        if (c4PacketSz < kHAIOmniTCPS::c4MinPacketSize)
        {
            facHAIOmniTCPS().ThrowErr
            (
                CID_FILE
                , CID_LINE
                , kOmniTCPSErrs::errcProto_PartialPacket
                , tCIDLib::ESeverities::Failed
                , tCIDLib::EErrClasses::Format
            );
        }
    ...
    if (c2Seq && (c2Seq != c2ExpSeq))
    {
        ...
        (
            CID_FILE
            , CID_LINE
            , kOmniTCPSErrs::errcProto_OutOfSync
            , tCIDLib::ESeverities::Failed
    ...
    //
    if (c1PacketType == kHAIOmniTCPS::c1PackType_OmniMsg)
    {
        //
        ...
        if (c2MsgCRC && (c2MsgCRC != c2CalcedCRC) && (c2MsgCRC != c2FlippedCRC))
        {
            ...
            (
                CID_FILE
                , CID_LINE
                , kOmniTCPSErrs::errcProto_BadCRC
                , tCIDLib::ESeverities::Failed
                , tCIDLib::EErrClasses::Format
            );
        }
        ...
        if (mbufToFill[kHAIOmniTCPS::c4MsgTypeOfs] == kHAIOmniTCPS::c1MsgType_Nak)
        {
            IncNakCounter();
            facHAIOmniTCPS().ThrowErr
            (
     ...
     else
    {
        ...
        if (c4MsgBytes > kHAIOmniTCPS::c4MaxMsgSize)
        {
            IncBadMsgCounter();
            facHAIOmniTCPS().ThrowErr
            (
                CID_FILE
                , CID_LINE
                , kOmniTCPSErrs::errcProto_MsgTooBig
                , tCIDLib::ESeverities::Failed
                , tCIDLib::EErrClasses::Format
                , TCardinal(c4MsgBytes)
            );
...
//
tCIDLib::TBoolean
THAIOmniTCPSDriver::bValidateCode(  const   TString&            strCode
                                    ...
                                    ,       tCIDLib::TCard1&    c1CodeNum)
{
    ...
    //
    mbufWrite.PutCard1(kHAIOmniTCPS::c1STX, 0);
    mbufWrite.PutCard1(6, 1);
    mbufWrite.PutCard1(kHAIOmniTCPS::c1MsgType_ValidateCodeReq, 2);
    ...
    SendPacket(mbufWrite, kHAIOmniTCPS::c1PackType_OmniMsg, 10);

    // And we should get a code validation response
    #if CID_DEBUG_ON
    WaitReply(30000, kHAIOmniTCPS::c1MsgType_ValidateCodeReply, 0, mbufRead);
    #else
    WaitReply(0, kHAIOmniTCPS::c1MsgType_ValidateCodeReply, 0, mbufRead);
    #endif

    c1CodeNum = mbufRead[kHAIOmniTCPS::c4FirstDBOfs];
    return (c1CodeNum != 0);
}
...
tCIDLib::TBoolean
THAIOmniTCPSDriver::bWaitAck(const tCIDLib::TBoolean bThrowIfTimeout)
{
    // Get a buffer to use
    ...
    try
    {
        ...
        WaitReply
        (
            kHAIOmniTCPS::c4WaitAck, kHAIOmniTCPS::c1MsgType_Ack, 0, mbufRead
        );
    }
    ...
    catch(TError& errToCatch)
    {
        ...
        //  it.
        //
        if (errToCatch.bCheckEvent( facHAIOmniTCPS().strName()
                                    , kOmniTCPSErrs::errcProto_Timeout))
        {
            if (bThrowIfTimeout)
                throw;
...
//
tCIDLib::TCard2 THAIOmniTCPSDriver::c2CalcCRC(const TMemBuf& mbufSrc)
{
...
tCIDLib::TCard4
THAIOmniTCPSDriver::c4ToOmniTemp(const  tCIDLib::TInt4      i4Convert
...
//
tCIDLib::TCard4
THAIOmniTCPSDriver::c4EncryptPayload(const  tCIDLib::TCard1* const  pc1Payload
                                    ...
                                    ,       TMemBuf&                mbufMsg)
{
    ...
    if (c4PLBytes)
    {
        ...
        if (c4Partial)
        {
            TRawMem::SetMemBuf(m_ac1Plain, kCIDLib::c1MinCard, 16);
...
//
tCIDLib::TInt4
THAIOmniTCPSDriver::i4FromOmniTemp( const   tCIDLib::TCard4     c4OmniTemp
                                    , const tCIDLib::TBoolean   bCelsiusFmt
...
//  done nough to have a special one to handle the format.
//
tCIDLib::TVoid
THAIOmniTCPSDriver::SendObjStatusQuery( const   tCIDLib::TCard1 c1ObjType
                                        ...
                                        , const tCIDLib::TCard2 c2EndNum)
{
    ...
    mbufWrite.PutCard1(kHAIOmniTCPS::c1STX, 0);
    mbufWrite.PutCard1(6, 1);
    mbufWrite.PutCard1(kHAIOmniTCPS::c1MsgType_ExtObjStatusReq, 2);
...
//
tCIDLib::TVoid
THAIOmniTCPSDriver::SendOmniCmd(const   tCIDLib::TCard1 c1Cmd
                                ...
                                , const tCIDLib::TCard2 c2Parm2)
{
    ...
    mbufWrite.PutCard1(kHAIOmniTCPS::c1MsgType_SendCmd, 2);
    ...
    SendPacket(mbufWrite, kHAIOmniTCPS::c1PackType_OmniMsg, 9);
}

tCIDLib::TVoid THAIOmniTCPSDriver::SendOmniMsg(const tCIDLib::TCard1 c1Type)
{
    // Get a buffer to use
    THeapBufPool::TElemJan janWrite(&m_splBufs, 64);
    TMemBuf& mbufWrite = *janWrite;
    ...
    SendPacket(mbufWrite, kHAIOmniTCPS::c1PackType_OmniMsg, 5);
}

tCIDLib::TVoid
THAIOmniTCPSDriver::SendOmniMsg(const   tCIDLib::TCard1 c1Type
                                , const tCIDLib::TCard1 c1Data)
{
    ...
    SendPacket(mbufWrite, kHAIOmniTCPS::c1PackType_OmniMsg, 6);
}
                                ...
                                , const tCIDLib::TCard1 c1Data2)
{
    ...
    SendPacket(mbufWrite, kHAIOmniTCPS::c1PackType_OmniMsg, 7);
}
                                ...
                                , const tCIDLib::TCard1 c1Data3)
{
    ...
    SendPacket(mbufWrite, kHAIOmniTCPS::c1PackType_OmniMsg, 8);
}


// Sends a packet with payload data
tCIDLib::TVoid
THAIOmniTCPSDriver::SendPacket( const   TMemBuf&        mbufToSend
...
tCIDLib::TVoid
THAIOmniTCPSDriver::SendPacket( const   tCIDLib::TCard1* const  pc1ToSend
                                , const tCIDLib::TCard1         c1MsgType
                                , const tCIDLib::TCard4         c4SendBytes)
{
    if (c4SendBytes > kHAIOmniTCPS::c4MaxMsgSize)
    {
        facHAIOmniTCPS().ThrowErr
        (
            CID_FILE
...
tCIDLib::TVoid THAIOmniTCPSDriver::SendPacket(const tCIDLib::TCard1 c1MsgType)
{
    // Get a buffer to use
    THeapBufPool::TElemJan janWrite(&m_splBufs, 64);
    TMemBuf& mbufWrite = *janWrite;
...
//
tCIDLib::TVoid
THAIOmniTCPSDriver::WaitReply(  const   tCIDLib::TCard4     c4WaitFor
                                ...
                                ,       TMemBuf&            mbufToFill)
{
    ...
    (
        c4WaitFor ? c4WaitFor : kHAIOmniTCPS::c4WaitReply
    );
    ...
    while (enctCur < enctEnd)
    {
        ...
        if (bGotONe)
        {
            ...
            c1OmniType = mbufToFill[kHAIOmniTCPS::c4MsgTypeOfs];
            if ((c1Type == kHAIOmniTCPS::c1PackType_OmniMsg) && (c1OmniType == c1ExpMsg))
            {
                //
                ...
                //
                if ((c1OmniType != kHAIOmniTCPS::c1MsgType_ExtObjStatusRep)

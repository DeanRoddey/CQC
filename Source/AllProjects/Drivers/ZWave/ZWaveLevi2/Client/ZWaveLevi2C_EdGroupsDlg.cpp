//
// FILE NAME: ZWaveLevi2C_EdGroupsDlg.cpp
//
// AUTHOR: Dean Roddey
//
// CREATED: 08/21/2014
...
TZWaveEdGroupsDlg::TZWaveEdGroupsDlg() :
    ...
    m_colEdit(kZWaveLevi2Sh::c4MaxGroupId)
...
// ---------------------------------------------------------------------------
tCIDLib::TBoolean
TZWaveEdGroupsDlg::bRunDlg( const   TWindow&            wndOwner
                            ...
                            ,       TZWaveDrvConfig&    dcfgEdit)
{
    ...
    (
        wndOwner, facZWaveLevi2C(), kZWaveLevi2C::ridDlg_EdGrps
    );

    if (c4Res == kZWaveLevi2C::ridDlg_EdGrps_Save)
    {
        //
...
tCIDLib::TBoolean TZWaveEdGroupsDlg::bCreated()
{
    ...
    CastChildWnd(*this, kZWaveLevi2C::ridDlg_EdGrps_Cancel, m_pwndCancel);
    CastChildWnd(*this, kZWaveLevi2C::ridDlg_EdGrps_Delete, m_pwndDelete);
    CastChildWnd(*this, kZWaveLevi2C::ridDlg_EdGrps_DeleteAll, m_pwndDeleteAll);
    CastChildWnd(*this, kZWaveLevi2C::ridDlg_EdGrps_List, m_pwndList);
    CastChildWnd(*this, kZWaveLevi2C::ridDlg_EdGrps_Save, m_pwndSave);
...
{
    TYesNoBox msgbAsk(facZWaveLevi2C().strMsg(kZW2CMsgs::midQ_DelAllUnits));
...
tCIDLib::TVoid TZWaveEdGroupsDlg::DeleteGroup()
{
    ...
    if ((c4Index != kCIDLib::c4MaxCard) && !m_colEdit[c4Index].bIsEmpty())
    {
        TYesNoBox msgbAsk
        (
            facZWaveLevi2C().strMsg(kZW2CMsgs::midQ_DelUnit, m_colEdit[c4Index])
        );
...
tCIDLib::TVoid TZWaveEdGroupsDlg::EditGroup()
{
    ...
    {
        TString strCur = m_colEdit[c4Index];
        TString strNew;
        const tCIDLib::TBoolean bRes = facCIDWUtils().bGetText
        (
            *this, facZWaveLevi2C().strMsg(kZW2CMsgs::midPrompt_GroupName), strCur, strNew
        );
        ...
        if (bRes && !strNew.bIsEmpty())
        {
            ...
            for (; c4FoundAt < kZWaveLevi2Sh::c4MaxGroupId; c4FoundAt++)
            {
            ...
            if (c4FoundAt == c4Index)
            {
             ...
             else if (c4FoundAt != kZWaveLevi2Sh::c4MaxGroupId)
            {
                // It's a dup, so reject it
                TErrBox msgbDup
                (
                    facZWaveLevi2Sh().strMsg(kZW2ShErrs::errcCfg_DupGroup, strNew)
                );
                msgbDup.ShowIt(*this);
            }
             else
...
TZWaveEdGroupsDlg::eClickHandler(TButtClickInfo& wnotEvent)
{
    if (wnotEvent.widSource() == kZWaveLevi2C::ridDlg_EdGrps_Cancel)
        EndDlg(kZWaveLevi2C::ridDlg_EdGrps_Cancel);
    else if (wnotEvent.widSource() == kZWaveLevi2C::ridDlg_EdGrps_Delete)
        DeleteGroup();
    else if (wnotEvent.widSource() == kZWaveLevi2C::ridDlg_EdGrps_DeleteAll)
        DeleteAllGroups();
    else if (wnotEvent.widSource() == kZWaveLevi2C::ridDlg_EdGrps_Save)
        EndDlg(kZWaveLevi2C::ridDlg_EdGrps_Save);
...
tCIDCtrls::EEvResponses TZWaveEdGroupsDlg::eLBHandler(TListChangeInfo& wnotEvent)
{
    ...
    {
        if (wnotEvent.widSource() == kZWaveLevi2C::ridDlg_EdGrps_List)
            EditGroup();
    }
    return tCIDCtrls::EEvResponses::Handled;
}
